# Terminal multiplexer (tmux) with plugins and theme-friendly status (opt-in)
{
  config,
  lib,
  pkgs,
...
in {
  options.features.cli.tmux.enable = mkEnableOption "tmux multiplexer (plugins, Catppuccin)";
